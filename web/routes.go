...
func routes(
	...
	mux.Handle("GET /note/{id}/{slug}", authReq.then(handlers.NoteViewGet(tc, logger, handlers.GetSQLiteNotesRepo)))
	...
	mux.Handle("POST /note", authReq.then(handlers.NotePost(tc, logger, handlers.GetSQLiteNotesRepo)))
	mux.Handle("PUT /note/{id}/{slug}", authReq.then(handlers.NotePut(tc, logger, handlers.GetSQLiteNotesRepo)))
	mux.Handle("DELETE /note/{id}/{slug}", authReq.then(handlers.NoteDelete(tc, logger, handlers.GetSQLiteNotesRepo)))
	mux.Handle("GET /edit/{id}/{slug}", authReq.then(handlers.NoteEditGet(tc, logger, handlers.GetSQLiteNotesRepo)))

	mux.Handle("GET /section", authReq.then(handlers.SectionGet(tc, logger, handlers.GetSQLiteNotesRepo)))
	...
	mux.Handle("GET /index", authReq.then(handlers.TagGet(tc, logger, handlers.GetSQLiteNotesRepo)))
	...
	mux.Handle("GET /tag/{id}/{slug}", authReq.then(handlers.TagViewGet(tc, logger, handlers.GetSQLiteNotesRepo)))
